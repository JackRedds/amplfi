...
class MultiModalQTransform(Embedding):
    """
    MultiModal embedding network that embeds both time and frequency data.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.context_dim = time_context_dim + freq_context_dim + q_context_dim

        self.qt = SingleQTransform(
        ...
        self.q_transform_resnet = ResNet2D(
            in_channels=num_ifos,
            layers=q_layers,
            classes=q_context_dim,
            kernel_size=q_kernel_size,
            zero_init_residual=zero_init_residual,
            groups=groups,
            width_per_group=width_per_group,
            stride_type=stride_type,
    ...
    def forward(self, X):
        ...
        X_q = self.qt(strain)
        q_domain_embedded = self.q_transform_resnet(X_q)
        embedding = torch.concat(
            (time_domain_embedded, q_domain_embedded, frequency_domain_embedded),
            dim=1,
        )
        return embedding
